...
import com.eslamdev.weathroza.presentaion.alerts.views.components.alerts_selector_items.TimeBasedSelectorItem
    ...
    Scaffold(
        ...
        Column(
            ...
            SettingsSelector(modifier = Modifier.padding(horizontal = 16.dp)) {
                TimeBasedSelectorItem(
                    isSelected = selectedTab == AlertTab.TIME_BASED,
                    onClick = { selectedTab = AlertTab.TIME_BASED },
                )
                ContinuousSelectorItem(
                    isSelected = selectedTab == AlertTab.CONTINUOUS,
                    onClick = { selectedTab = AlertTab.CONTINUOUS },
                )
            }
            ...
            val filteredState = remember(uiState, selectedTab) {
                if (uiState is UiState.Success) {
                    val filtered = (uiState as UiState.Success).data.filter {
                        if (selectedTab == AlertTab.TIME_BASED) {
