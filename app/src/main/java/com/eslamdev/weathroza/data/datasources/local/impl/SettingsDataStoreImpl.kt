...
class SettingsDataStoreImpl(private val context: Context) : SettingsDataStore {
    ...
    override val settingsFlow: Flow<UserSettings> = context.dataStore.data
    ...
    override suspend fun saveLocationType(type: LocationType) {
    ...
    override suspend fun saveGpsLocation(lat: Double, lng: Double) {
        context.dataStore.edit {
            it[USER_LAT] = lat
            it[USER_LNG] = lng
    ...
    override suspend fun saveCityId(cityId: Long) {
        context.dataStore.edit { it[CITY_ID] = cityId }
    }
